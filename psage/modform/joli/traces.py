"""
Dimension of the spaces $J_{k,m}$ of Jacobi forms of
weight $k$ and (scalar)index $m$ on $\mathoperator{SL}(2,Z)$
and traces of $T(l) \circ W_n$ on these spaces.
...
TODO
    o intensive checking
    o skew-holomorphic not yet implemented for Eisenstein series
      and weight 1 cusp forms
    o dimensions and Poincare for Atkin-Leher subspaces,
...
from sz_trace_formula import (sz_s, ab, check)
from constants import *


def check_jac_params( k, m, eps):
    k = Integer(k)
    ...
    eps = Integer(eps)
    if eps != HOLOMORPHIC and eps != SKEW_HOLOMORPHIC:
        raise ValueError, "typus %s: must be a %d o%d" \
              % ( str(eps), HOLOMORPHIC, SKEW_HOLOMORPHIC)
    return k, m, eps


def dimension_jac_eis_series( k, m, eps = -1):
    r"""
    OUTPUT
    ...
    EXAMPLES
    """
    k, m, eps = check_jac_params( k, m, eps)
    if -1 != eps:
...
def trace_jacobi_cusp_forms( l, n, k, m, eps = -1):
    r"""
    OUTPUT
    ...
    if k <= 1:
        return 0
    return (sz_s(k,m,l,n) - eps*(-1)**(k%2)*sz_s(k,m,l,m//n))/2
...
def poincare_poly_jac_forms( m, var):
    ...
    EXAMPLES
    """
    k, m, eps = check_jac_params( 0, m, -1)
    x = ZZ[var].gen();
